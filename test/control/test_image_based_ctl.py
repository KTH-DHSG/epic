...
class TestMPRVS(unittest.TestCase):
    def setUp(self):
        ...
        self.sim_time_steps = 300  # 50 seconds at dt of 0.1
        ...
        q2 = get_random_quat_around(np.array([0, 0, 0, 1]), 20)
        q3 = get_random_quat_around(np.array([0, 0, 0, 1]), 20)
        self.f1.set_state(np.concatenate((np.array([0.2, 0.2, 0.1]), q2)))
        self.f2.set_state(np.concatenate((np.array([0.3, -0.2, 0.1]), q3)))
        ...
        # Add leader motion
        self.leader_u = np.array([[0.00, 0.0, -0.01, 0, 0, 0.001]]).T  # Leader's control input
