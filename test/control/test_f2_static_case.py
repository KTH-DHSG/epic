...
class TestMPRVS(unittest.TestCase):
    def setUp(self):
        ...
        f2_p = np.random.randn(3, 1)
        f2_q = np.random.randn(4, 1)
        f2_q = f2_q / np.linalg.norm(f2_q)

        leader = np.concatenate((leader_p, leader_q), axis=0).reshape((7,))
        f1 = np.concatenate((f1_p, f1_q), axis=0).reshape((7,))
        f2 = np.concatenate((f2_p, f2_q), axis=0).reshape((7,))
        ...
        self.f2.set_controller(
            "ibfc", [self.leader, self.f1], config="f2_mpc.json", method="ibvs"
        )
        ...
        # Simulation steps
        self.sim_time_steps = 100  # 2 seconds at dt of 0.1

        # Ensure zero control inputs to leader and follower - can be adjusted
        self.leader_u = np.array([[0.000, 0.00, 0.0, 0, 0, 0.0]]).T
        self.f1_u = np.array([[0.000, 0.00, 0.0, 0, 0, 0.0]]).T
