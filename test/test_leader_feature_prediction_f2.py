...
from epic.utils.veronese import (
    lift_image_points,
    lift_mat_to_veronese,
    lift_vec_to_veronese,
)
...
class TestMPRVS(unittest.TestCase):
    ...
    def test_lifted_epipolar_condition_perspective_parabolic(self):
        ...
        x_px_points_h, xp_v_framework, (x_depth, x_delta) = (
            self.leader.get_image_points(self.w_points_h)
        )
        ...
        assert np.allclose(
            yp_v, yp_v_framework
        ), "Follower image points are not the same"
